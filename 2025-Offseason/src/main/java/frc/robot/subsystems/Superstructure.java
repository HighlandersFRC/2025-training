...
public class Superstructure extends SubsystemBase {
    ...
    private void applyStates() {
        switch (currentSuperState) {
            case DEFAULT:
                ...
                handleDefaultState();
                break;
            case ZERO: // TODO: make ts work gng
                break;
            case OUTAKE_IDLE:
                break;
            ...
            case HANDOFF:
                if (straightenator.isFar() && !manipulator.hasCoralSemiSticky()) {
                    ...
                    if (Constants.metersToInches(elevator.getElevatorPosition()) < Constants.Elevator.HANDOFF_LOW
                            + 1.0) {
                        handleHandoffState();
                    }
                } else
    ...
    public boolean placedCoralL2() {
        return Math
                .abs(Math.abs(arm.getArmDegrees())
                        - Constants.Arm.L2_Score) < (5.0 / 360.0);
    }

    public boolean placedCoralL4() {
        return Math
                .abs(Math.abs(arm.getArmDegrees())
                        - Constants.Arm.L4_Score) < (5.0 / 360.0);
    }
