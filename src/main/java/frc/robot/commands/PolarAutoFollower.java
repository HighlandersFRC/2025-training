    ...
    public PolarAutoFollower(JSONObject auto, HashMap<String, Supplier<Command>> commandMap,
        ...
        for (int i = 0; i < schedule.length(); i++) {
            if (!schedule.getJSONObject(i).getBoolean("branched")) {
                ...
                addCommands(new PolarPathFollower(paths.getJSONObject(pathIdx), commandMap,
                        conditionMap));
            } else {
                ...
                PolarAutoFollower onTrueFollower = new PolarAutoFollower(onTrueObj,
                        commandMap, conditionMap);
                PolarAutoFollower onFalseFollower = new PolarAutoFollower(onFalseObj,
                        commandMap, conditionMap);
                addCommands(new ConditionalCommand(onTrueFollower, onFalseFollower,
                        condition));
